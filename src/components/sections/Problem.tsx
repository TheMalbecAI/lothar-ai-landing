...
export function Problem() {
  return (
    <SectionWrapper className="bg-crimson-subtle">
      ...
      <StaggerChildren className="mt-16 grid grid-cols-1 md:grid-cols-3 gap-8 md:auto-rows-[1fr]">
        {PROBLEM_CONTENT.cards.map((card, i) => (
          <div
            key={card.title}
            className="bg-surface-elevated rounded-xl p-8 border border-surface-border flex flex-col h-full"
          >
            ...
            <p className="text-cream-muted leading-relaxed text-sm mt-auto">
              {card.description}
            </p>
          </div>
        ))}
